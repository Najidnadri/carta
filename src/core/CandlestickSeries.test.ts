...
describe("CandlestickSeries.priceRangeInWindow", () => {
  let store: DataStore;
  ...
  beforeEach(() => {
    ...
    series = new CandlestickSeries({ channel: CHANNEL });
  ...
  it("returns {min(low), max(high)} over visible records", () => {
  ...
  it("returns null before query context is attached", () => {
    const detached = new CandlestickSeries({ channel: CHANNEL });
